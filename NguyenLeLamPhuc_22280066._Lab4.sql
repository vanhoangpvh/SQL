...
insert into MATHANG values(22,'Gustaf''s'' Knackebr�d',9,21,0)
...
insert into MATHANG values(55,'P�t� chinois',25,24,0)
...
--Q2
select MH.MaMH, MH.TenMH, MH.DonGia, avg(CT.DonGia) as 'Don gia dat hang trung binh', count(CT.MaDDH) as 'Tong so luong mat hang'
from MATHANG as MH join CHITIETDDH as CT on MH.MaMH=CT.MaMH
join DONDATHANG as DDH on CT.MaDDH=DDH.MaDDH and datepart(month,DDH.NgayDatHang)=7 and datepart(year,DDH.NgayDatHang)=2012
group by MH.MaMH, MH.TenMH, MH.DonGia

--Q3
with TongSoLuong (MaMH,TongSL) as (
	select MaMH, sum(CT.SoLuong)
	from CHITIETDDH as CT
	group by MaMH
)
select MH.MaMH, MH.TenMH, sum(CT.SoLuong) as 'Tong so luong'
from MATHANG as MH join CHITIETDDH as CT on MH.MaMH=CT.MaMH
join TongSoLuong as TSL on CT.MaMH=TSL.MaMH
group by MH.MaMH, MH.TenMH
having sum(CT.SoLuong)=
	(select max(TSL.TongSL) from TongSoLuong as TSL)
...
--Q5
select KH.MaKH, KH.HoTen, count(DDH.MaDDH) as 'Tong so luong don dat hang'
from KHACHHANG as KH left join DONDATHANG as DDH on KH.MaKH=DDH.MaKH
group by KH.MaKH, KH.HoTen

--Q6
select NCC.MaNCC, NCC.TenNCC, NCC.Email, count(MH.MaMH) as 'Tong so mat hang lon hon 30'
from NHACUNGCAP as NCC left join MATHANG as MH on NCC.MaNCC=MH.MaNCC and MH.DonGia>30
