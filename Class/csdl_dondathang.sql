...
WHERE kh.hoten LIKE '% L%';

--Cau 3
SELECT *
FROM dondathang as ddh
WHERE CURRENT_TIMESTAMP - ddh.ngaydathang < INTERVAL '3 months';
...
WHERE nhacungcap.quocgia = 'Germany';
-- Cau 5
SELECT mathang.mamh, mathang.tenmh , mathang.dongia 
FROM mathang
INNER JOIN chitietddh ON chitietddh.mamh = mathang.mamh 
INNER JOIN dondathang ON chitietddh.maddh = dondathang.maddh 
WHERE EXTRACT(MONTH FROM dondathang.ngaydathang) = 7 and EXTRACT(YEAR FROM dondathang.ngaydathang) = 2012
ORDER BY mathang.dongia DESC;
--Cau 6
SELECT mathang.mamh 
FROM mathang
EXCEPT
SELECT dondathang.maddh 
FROM dondathang 
WHERE EXTRACT(MONTH FROM dondathang.ngaydathang) = 7 and EXTRACT(YEAR FROM dondathang.ngaydathang) = 2012
