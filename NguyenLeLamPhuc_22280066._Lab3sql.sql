...
select *
from DONDATHANG
where datediff(month,NgayDatHang,current_timestamp) = 3;

-- Q4 
select NCC.MaNCC, TenNCC, MaMH, TenMH
from NHACUNGCAP as NCC, MATHANG as MH
where NCC.MaNCC= MH.MaNCC and QuocGia='Germany' ;

-- Q5
select MH.MaMH, MH.TenMH, MH.DonGia
from CHITIETDDH as CT, MATHANG as MH
where CT.MaMH=MH.MaMH and CT.MaDDH IN (
select MaDDH
from DONDATHANG as DDH
where datepart(month,DDH.NgayDatHang)=7 and datepart(year,DDH.NgayDatHang)=2012
)
